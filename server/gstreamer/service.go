...
func (s *Service) GetOrAdd(hash string, fileID string, audio int) (*Task, error) {
	...
	for {
		...
		callKey := fmt.Sprintf("%s:%s:%d", hash, fileID, audio)
		value, err, _ := s.taskCalls.Do(callKey, func() (any, error) {
...
func shouldUseCueTimeline(conf Config, probe ProbeInfo) bool {
	// Container CUEs are seekpoints, not guaranteed IDR keyframes.
	// Attempting to read them forces TorrServer to download pieces from the tail of the torrent,
	// causing severe 15-45s startup delays, and causes segment crashes when keyframes don't align.
	// Non-cue mode uses keyframe-adaptive segmentation that works reliably across all containers.
	return false
}
