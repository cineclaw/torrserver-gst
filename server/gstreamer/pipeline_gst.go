...
const (
	...
	pipelineSeekTimeout  = 35 * time.Second
...
func videoIsTranscoded(conf Config, probe ProbeInfo) bool {
	...
	// Web browsers (Chrome, Safari, Firefox) DO NOT support 10-bit H.264 (Hi10P / High 4:4:4).
	// MediaSource throws decoding failure or shows black screen with no audio.
	// We MUST transcode 10-bit H.264 to standard 8-bit H.264 for seamless browser playback.
	if probe.IsH264() && probe.Video() != nil && (probe.Video().BitDepth > 8 || strings.Contains(strings.ToLower(probe.Video().Codec), "4:4:4")) {
		return true
	}
	switch {
	case probe.IsH264():
...
func (r *gstRunner) Seek(seconds float64) bool {
	...
	var err error
	if reuse {
		actualSeconds, err = r.reusePipeline(seconds, accurate, pipelineSeekTimeout)
	} else {
...
func (r *gstRunner) reusePipeline(seconds float64, accurate bool, waitTimeout time.Duration) (float64, error) {
	...
	waitResult := gstRuntime.elementGetState(r.pipeline, waitTimeout)
...
func (r *gstRunner) EnsureInit(ctx context.Context, audio int, startIndex int) error {
	...
	if targetAudio := validAudioIndex(r.task.Probe, audio); targetAudio >= 0 && r.audioIndex != targetAudio {
		r.audioIndex = targetAudio
		r.freezeAtPosition(r.segmentStartSeconds(startIndex))
	}
...
func (r *gstRunner) GetSegment(ctx context.Context, index int, audio int) (Segment, error) {
	...
	if targetAudio := validAudioIndex(r.task.Probe, audio); targetAudio >= 0 && r.audioIndex != targetAudio {
		r.audioIndex = targetAudio
		r.freezeAtPosition(r.segmentStartSeconds(index))
	}
...
func (r *gstRunner) startPipeline(seconds float64) (float64, error) {
	...
	if seconds > 0 {
		...
		asyncDone, err := gstRuntime.waitForSeekDone(bus, pipelineSeekTimeout)
		if err != nil {
			cleanup()
		...
		waitResult := gstRuntime.elementGetState(pipeline, pipelineSeekTimeout)
		switch waitResult {
