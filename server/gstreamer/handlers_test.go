...
func TestBuildTaskPlaylistIncludesAudioParam(t *testing.T) {
	task := &Task{
		Config: Config{SegmentSeconds: 6},
		Probe: ProbeInfo{
			DurationNS: 18 * 1_000_000_000,
		},
	}
	playlist := buildTaskPlaylist(task, 0, 2)
	if !strings.Contains(playlist, "init.mp4?audio=2") {
		t.Fatalf("playlist missing init.mp4?audio=2: %s", playlist)
	}
	if !strings.Contains(playlist, "seg/0.m4s?audio=2") {
		t.Fatalf("playlist missing seg/0.m4s?audio=2: %s", playlist)
	}

	// Subtitle playlist should cleanly parse the segment indices even with ?audio=2
	subPlaylist := buildSubtitlePlaylist(task, 1)
	if !strings.Contains(subPlaylist, "1/0.vtt") {
		t.Fatalf("subPlaylist missing 1/0.vtt: %s", subPlaylist)
	}
}
