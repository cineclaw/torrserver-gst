...
type Torrent struct {
	...
	warmupBeginPiece atomic.Int64
	warmupEndPiece   atomic.Int64
...
// SetWarmupRange signals that an in-flight playback / warmup is targeting the specific piece range [beginPiece, endPiece)
func (t *Torrent) SetWarmupRange(active bool, beginPiece, endPiece int) {
	t.warmupBeginPiece.Store(int64(beginPiece))
	t.warmupEndPiece.Store(int64(endPiece))
...
// SetWarmupActive is called by the TorrServer layer to signal
...
// warmed, scoped to at most maxPieces from the start of that file. Returns ok=false if
// metadata isn't resolved yet.
func (t *Torrent) warmupPieceRange(maxPieces int) (begin, end pieceIndex, ok bool) {
	if !t.haveInfo() {
		return 0, 0, false
	}
	// Check if explicit piece range was set via SetWarmupRange
	b := int(t.warmupBeginPiece.Load())
	e := int(t.warmupEndPiece.Load())
	numP := t.numPieces()
	if e > b && b >= 0 && b < numP {
		begin = pieceIndex(b)
		end = pieceIndex(e)
	...
	// Handle 1-based indexing from TorrServer or 0-based
...
const (
	...
	// hedgeCircuitBreakerThreshold: raised to 60 to prevent false-positive trips during
	// initial 16-chunk pipelined burst on slower connections.
	hedgeCircuitBreakerThreshold = 60
	hedgeCircuitBreakerWindow    = 60 * time.Second
	hedgeCircuitBreakerCooldown  = 30 * time.Second
...
func (t *Torrent) checkAndFireHedges() {
	var wBegin, wEnd pieceIndex
	var wOk bool
	if t.warmupActive.Load() {
		wBegin, wEnd, wOk = t.warmupPieceRange(hedgeWarmupPieceWindow)
	}

	var pBegin, pEnd pieceIndex
	var pOk bool
	if t.playbackPressureActive.Load() {
		pBegin, pEnd, pOk = t.playbackPressureRange()
	}
	...
	for r, rs := range t.requestState {
		...
		inRange := (wOk && pieceIdx >= wBegin && pieceIdx < wEnd) ||
			(pOk && pieceIdx >= pBegin && pieceIdx < pEnd)
		if !inRange && wOk {
			fileEnd := pieceIndex(t.warmupEndPiece.Load())
			fileBegin := pieceIndex(t.warmupBeginPiece.Load())
			if fileEnd > fileBegin && fileEnd > 2 && pieceIdx >= fileEnd-4 && pieceIdx < fileEnd {
				inRange = true
			}
		}
		if !inRange {
			continue // outside active piece ranges - not an in-demand request
		}
...
func (t *Torrent) peerIsSolePieceSource(c *PeerConn) bool {
	ranges := make([][2]pieceIndex, 0, 2)
	if t.warmupActive.Load() {
		if begin, end, ok := t.warmupPieceRange(hedgeWarmupPieceWindow); ok {
			ranges = append(ranges, [2]pieceIndex{begin, end})
		}
	}
	if t.playbackPressureActive.Load() {
		if begin, end, ok := t.playbackPressureRange(); ok {
			ranges = append(ranges, [2]pieceIndex{begin, end})
		}
	}
	if len(ranges) == 0 {
...
func (t *Torrent) openNewConns() (initiated int) {
	...
	for t.peers.Len() != 0 {
		...
		// but only if we already have sufficient established connections (>= 20).
		if t.cl.config.AggressivePeerManagement && len(t.conns) >= 20 && len(t.churnCooldown) > 0 {
