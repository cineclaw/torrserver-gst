...
func (t *Torrent) churnIfUselessForWarmup(pc *PeerConn) {
	...
	t.cl.lock()
	// Never drop peers when swarm connection count is low (< 25). Every peer is critical in small swarms.
	if len(t.conns) < minConnsToChurn {
		t.cl.unlock()
		return
	}
	now := time.Now()
	...
	// (arrived just after the window closed) rather than genuine uselessness, so give it one
	...
	t.cl.lock()
	defer t.cl.unlock()
	if pc.closed.IsSet() || !t.warmupActive.Load() || len(t.conns) < minConnsToChurn {
		return // already gone, warmup finished, or connection count fell below churn threshold
	}
	// Never drop a complete seeder
	...
	// Also check tail pieces of the file (cues / index at file end needed for probing)
	...
	if e > b {
		...
		for i := tailStart; i < e; i++ {
			if pc.peerHasPiece(pieceIndex(i)) {
				return
			}
		}
	}
